...
**disco.yml** - Configuration for Disco:
...
![GitHub authorization page](images/github_auth.png)
...
The deployment process might take a few minutes to complete. You'll see output showing the progress.
...
![API response in browser](images/api_response.png)
...
1. Modify the greeting in `main.py`:

```python
@app.get("/")
def read_root():
    return {"message": "Hello from Hetzner Cloud with FastAPI and Disco!"}
```
